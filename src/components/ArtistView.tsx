      ...
      .then((data) => {
        ...
        if (data) {
          ...
          if (Array.isArray(data.topSongs)) {
            const currentTarget = (data.name || artistName || '').toLowerCase().trim();
            const songs: Song[] = data.topSongs
              .filter((s: any) => {
                if (!currentTarget) return true;
                const rawArtist = (typeof s.artist === 'string' ? s.artist : s.artist?.name || '').toLowerCase();
                const title = (s.name || s.title || '').toLowerCase();
                if (currentTarget.includes('seventeen')) {
                  const seventeenHits = [
                  ...
                  if (seventeenHits.some(h => title.includes(h))) return true;
                  return rawArtist.includes('seventeen') || title.includes('seventeen');
                }
                if (rawArtist && rawArtist.includes(currentTarget)) return true;
                if (title.includes(currentTarget)) return true;
                // If artist is completely different known artist, reject
                if (rawArtist && !rawArtist.includes(currentTarget)) return false;
                return true;
              })
              .map((s: any) => ({
